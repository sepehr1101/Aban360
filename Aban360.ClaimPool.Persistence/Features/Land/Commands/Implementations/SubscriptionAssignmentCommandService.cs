...
namespace Aban360.ClaimPool.Persistence.Features.Land.Commands.Implementations
{
    internal sealed class SubscriptionAssignmentCommandService : AbstractBaseConnection, ISubscriptionAssignmentCommandService
    {
        ...
        public async Task Update(SubscriptionUpdateDto updateDto)
        {
            ...
            var @updateParams = new
            {
                ...
                ToDayDateJalali = updateDto.ToDayDateJalali.Substring(2),
            ...
            {
                //billId = updateDto.BillId,
                ToDayDateJalali = updateDto.ToDayDateJalali,
                //id = updateDto.Id,    
                zoneId = customerInfo.ZoneId,
                customerNumber = customerInfo.CustomerNumber,
            };
            ...
            {
                var insertResult = await _sqlReportConnection.ExecuteAsync(insertQuery, updateDto);
        ...
        private string GetInsertQuery(string dbName)
        {
            return @$";With cte as
                    ...
                    SELECT 
                        ...
                        @OtherUnit, @DomesticUnit, @HouseholdNumber, @CommertialUnit, date_sabt, arse, aian, aian_mas,
                        aian_tej, ask_ab, inst_ab, ask_fas, inst_fas, @Address, @Plaque, bed_bes, @IsSpecial,
                        ...
                        @PhoneNumber, @MobileNumber, @NationalCode, oRadif, sif_5, sif_6, sif_7, sif_8, bill_id, MOJAVZ,
                        DATEINS, c20, balansing, tmp_date_sabt, tmp_ask_ab, tmp_ask_fas, tmp_inst_ab,
                        tmp_inst_fas, tmp_g_inst_ab, tmp_g_inst_fas, tmp_date_sabt, @EmptyUnit, Senf, @HouseholdDateJalali
                    FROM cte m
                    WHERE 
                        m.town=@zoneId AND
        ...
        private string GetUpdateQuery(string dbName)//todo: date_KHANE ?
        {
            return @$"UPDATE [{dbName}].dbo.members
                     SET 
                         ...
                         family = @surName,
                         address = @address,
                         date_KHANE = @ToDayDateJalali,
                         DATEINS=@ToDayDateJalali,
						 POST_COD=@postalCode
                     WHERE 
                        id=@id AND
						bill_id=@billId AND
