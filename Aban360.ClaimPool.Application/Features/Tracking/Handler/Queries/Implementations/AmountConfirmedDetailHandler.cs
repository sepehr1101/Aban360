...
namespace Aban360.ClaimPool.Application.Features.Tracking.Handler.Queries.Implementations
{
    internal sealed class AmountConfirmedDetailHandler : IAmountConfirmedDetailHandler
    {
        ...
        public async Task<AmountConfirmedOutputDto> Handle(TrackingDetailGetDto inputDto, CancellationToken cancellationToken)
        {
            await Validation(inputDto, cancellationToken);
            AmountConfirmedOutputDto data = await _trackingDetailQueryService.GetAmountConfirmed(inputDto);
            return data;
        }
