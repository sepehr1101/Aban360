...
namespace Aban360.ClaimPool.Application.Features.Tracking.Handler.Queries.Implementations
{
    internal sealed class ExamineTimeSetDetailHandler : IExamineTimeSetDetailHandler
    {
        ...
        public async Task<ExamineTimeSetOutputDto> Handle(TrackingDetailGetDto inputDto, CancellationToken cancellationToken)
        {
            await Validation(inputDto, cancellationToken);
            ExamineTimeSetOutputDto data = await _trackingDetailQueryService.GetExamineTimeSetDto(inputDto);          
            return data;
        }
