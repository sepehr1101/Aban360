using Aban360.ClaimPool.Application.Features.Metering.Handlers.Commands.Create.Contracts;
using Aban360.ClaimPool.Domain.Features.Metering.Dto.Commands;
using Aban360.ClaimPool.Domain.Features.Metering.Entities;
using Aban360.ClaimPool.Persistence.Features.Metering.Commands.Contracts;
using Aban360.Common.Extensions;
using AutoMapper;

namespace Aban360.ClaimPool.Application.Features.Metering.Handlers.Commands.Create.Implementations
{
    internal sealed class WaterMeterInstallationStructureCreateHandler : IWaterMeterInstallationStructureCreateHandler
    {
        private readonly IMapper _mapper;
        private readonly IWaterMeterInstallationStructureCommandService _waterMeterInstallationStructureCommandService;
        public WaterMeterInstallationStructureCreateHandler(
            IMapper mapper,
            IWaterMeterInstallationStructureCommandService waterMeterInstallationStructureCommandService)
        {
            _mapper = mapper;
            _mapper.NotNull(nameof(_mapper));

            _waterMeterInstallationStructureCommandService = waterMeterInstallationStructureCommandService;
            _waterMeterInstallationStructureCommandService.NotNull(nameof(_waterMeterInstallationStructureCommandService));
        }

        public async Task Handle(WaterMeterInstallationStructureCreateDto createDto, CancellationToken cancellationToken)
        {
            var waterMeterInstallationStructure = _mapper.Map<WaterMeterInstallationStructure>(createDto);
            await _waterMeterInstallationStructureCommandService.Add(waterMeterInstallationStructure);
        }
    }
}
