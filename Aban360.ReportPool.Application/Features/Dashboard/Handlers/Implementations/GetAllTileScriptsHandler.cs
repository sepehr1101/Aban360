...
{
    internal sealed class GetAllTileScriptsHandler : IGetAllTileScriptsHandler
    {
        private readonly ITileScriptService _tileScriptService;

        public GetAllTileScriptsHandler(ITileScriptService service)
        {
            _tileScriptService = service;
            _tileScriptService.NotNull(nameof(_tileScriptService));
        }

        public async Task<IEnumerable<TileScript>> Handle(CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            return await _tileScriptService.GetAll();
        }
    }
}
