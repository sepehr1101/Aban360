    ...
    internal sealed class FinancialStatementTotalHandler : IFinancialStatementTotalHandler
    {
        ...
        public async Task<ReportOutput<FinancialStatementHeaderOutputDto, FinancialStatementDataOutputDto>> Handle(FinancialStatementInputDto input, CancellationToken cancellationToken)
        {
            ...
            FinancialStatementHeaderOutputDto header = new()
            {
                ...
                CustomerCount = data?.Sum(s => s.CustomerCount) ?? 0,
                ConsumptionTotalUnit = data?.Sum(s => s.ConsumptionTotalUnit) ?? 0,
                DailyAverage = data?.Sum(s => s.DailyAverage) ?? 0,
                NetConsumption = data?.Sum(s => s.NetConsumption) ?? 0,
                NetAmount = data?.Sum(s => s.NetAmount) ?? 0,
                ReturnedConsumption = data?.Sum(s => s.ReturnedConsumption) ?? 0,
                ReturnedAmount = data?.Sum(s => s.ReturnedAmount) ?? 0,
                DiscountAmount = data?.Sum(s => s.DiscountAmount) ?? 0,
                RawConsumption = data?.Sum(s => s.RawConsumption) ?? 0,
                RawAmount = data?.Sum(s => s.RawAmount) ?? 0,
                RawAmountAverage = data?.Sum(s => s.RawAmountAverage) ?? 0,
                ConsumptionAverageInMonth = data?.Sum(s => s.ConsumptionAverageInMonth) ?? 0,
            };
