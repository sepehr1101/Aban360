...
{
    [Route("v1/service-link-payment-receivable-summary-by-zone")]
    public class ServiceLinkPaymentReceivableSummaryByZoneController : BaseController
    {
        private readonly IServiceLinkPaymentReceivableSummaryHandler _serviceLinkPaymentReceivable;
        private readonly IReportGenerator _reportGenerator;
        public ServiceLinkPaymentReceivableSummaryByZoneController(
        ...
        {
            inputDto.IsZone = true;
            await _reportGenerator.FireAndInform(inputDto, cancellationToken, _serviceLinkPaymentReceivable.Handle, CurrentUser, ReportLiterals.ServiceLinkPaymentReceivableSummary + ReportLiterals.ByZone, connectionId);
            return Ok(inputDto);
        }
        ...
        {
            int reportCode = 582;
            ReportOutput<WaterPaymentReceivableHeaderOutputDto, WaterPaymentReceivableSummaryDataOutputDto> result = await _serviceLinkPaymentReceivable.Handle(inputDto, cancellationToken);
