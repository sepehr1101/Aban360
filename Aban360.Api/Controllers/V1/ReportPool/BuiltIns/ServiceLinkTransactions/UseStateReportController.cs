...
namespace Aban360.Api.Controllers.V1.ReportPool.BuiltIns.ServiceLinkTransactions
{
    ...
    public class UseStateReportController:BaseController
    {
        ...
        [HttpPost, HttpGet]
        [Route("info")]
        [ProducesResponseType(typeof(ApiResponseEnvelope<ICollection<UseStateReportOutputDto>>),StatusCodes.Status200OK)]
        public async Task<IActionResult> GetIfo(UseStateReportInputDto inputDto, CancellationToken cancellationToken)
        {
            ICollection<UseStateReportOutputDto> useStates=await _useStateReportHandler.Handle(inputDto,cancellationToken);
            return Ok(useStates);
        }
    }
}
