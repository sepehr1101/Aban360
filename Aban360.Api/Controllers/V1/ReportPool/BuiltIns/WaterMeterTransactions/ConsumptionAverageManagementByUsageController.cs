...
{
    [Route("v1/consumption-average-management-summary-by-usage")]
    public class ConsumptionAverageManagementByUsageController : BaseController
    {
        private readonly IConsumptionAverageManagementSummaryByUsageHandler _consumptionAverageManagerHandler;
        private readonly IReportGenerator _reportGenerator;
        public ConsumptionAverageManagementByUsageController(
        ...
        [HttpPost]
        [Route("raw")]
        [ProducesResponseType(typeof(ApiResponseEnvelope<ReportOutput<ConsumptionAverageManagementHeaderOutputDto, ConsumptionAverageManagementSummaryDataOutputDto>>), StatusCodes.Status200OK)]
        public async Task<IActionResult> GetRaw(ConsumptionAverageManagementInputDto input, CancellationToken cancellationToken)
        {
            ReportOutput<ConsumptionAverageManagementHeaderOutputDto, ConsumptionAverageManagementSummaryDataOutputDto> result = await _consumptionAverageManagerHandler.Handle(input, cancellationToken);
            return Ok(result);
        }
        ...
        {
            int reportCode = 671;
            ReportOutput<ConsumptionAverageManagementHeaderOutputDto, ConsumptionAverageManagementSummaryDataOutputDto> calculationDetails = await _consumptionAverageManagerHandler.Handle(inputDto, cancellationToken);
