...
namespace Aban360.Api.Controllers.V1.TaxPool.MaaherTSP.Queries.Contracts
{
    ...
    public class SentInvoiceController : BaseController
    {
        ...
        [HttpPost]
        [Route("sent")]
        [ProducesResponseType(typeof(ApiResponseEnvelope<SentInvoiceRecieveDto>), StatusCodes.Status200OK)]
        public async Task<IActionResult> Send(ICollection<MaaherTSPInvoiceDto> inputDto, CancellationToken cancellationToken)
        {
            SentInvoiceRecieveDto sentInvoiceRecieve = await _sentInvoiceHandler.Handle(inputDto, cancellationToken);
