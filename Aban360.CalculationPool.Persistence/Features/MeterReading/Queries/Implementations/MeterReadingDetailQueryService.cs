...
namespace Aban360.CalculationPool.Persistence.Features.MeterReading.Queries.Implementations
{
    public sealed class MeterReadingDetailQueryService : AbstractBaseConnection, IMeterReadingDetailQueryService
    {
        ...
        public async Task<IEnumerable<MeterReadingDetailDataOutputDto>> Get(ChangeDateBatchInputDto inputDto)
        {
            string query = GetByFromToReadingNumberQuery();
            IEnumerable<MeterReadingDetailDataOutputDto> details = await _sqlReportConnection.QueryAsync<MeterReadingDetailDataOutputDto>(query, inputDto);
        ...
        private string GetWithFlowImportedIdQuery(bool? hasExcluded)
        {
            ...
            return $@"Select 
                        ...
                        m.TavizNumber,
                        m.LastMeterDateJalali PreviousMeterDateJalali,
                        m.LastMeterNumber PreviousMeterNumber,
                        m.LastMonthlyConsumption PreviousMonthlyConsumption,
                        m.LastConsumption PreviousConsumption,
                        m.LastCounterStateCode PreviousCounterStateCode,
                        c.Title PreviousCounterStateTitle,
                        m.LastSumItems PreviousSumItems,
                        m.SumItems,
                    ...
                    Left Join [Db70].dbo.CounterVaziat c
                        On c.MoshtarakinId = m.LastCounterStateCode
                    Where 
        ...
        private string GetByFromToReadingNumberQuery()
        {
            return $@"Select 
                        m.Id,
                        m.FlowImportedId,
					...
					Join Atlas.dbo.MeterReadingDetail m
						On m.FlowImportedId=mf.FirstFlowId
                    Left Join [Db70].dbo.T7 t7 
                        On m.BranchTypeId = t7.C0
                    Left Join [Db70].dbo.T41 t41
                        On m.UsageId = t41.C0
                    Left Join [Db70].dbo.CounterVaziat c
                        On c.MoshtarakinId = m.LastCounterStateCode
                    Where 
                        mf.Id=@MeterFlowId  AND
						m.ReadingNumber BETWEEN @FromReadingNumber AND @ToReadingNumber AND
