...
namespace Aban360.OldCalcPools.Persistence.Features.WaterReturn.Queries.Implementations
{
    internal sealed class MembersQueryService : AbstractBaseConnection, IMembersQueryService
    {
        ...
        public async Task<MemberGetDto> Get(ZoneIdAndCustomerNumber inputDto)
        {
            string dbName=GetDbName(inputDto.ZoneId);
            string query = GetInfoByCustomerNumberQuery(dbName);
            MemberGetDto data = await _sqlReportConnection.QueryFirstOrDefaultAsync<MemberGetDto>(query, inputDto);
            if (data is null || data.ZoneId <= 0)
            {
                throw new InvalidBillIdException(ExceptionLiterals.InvalidBillId);
            }
        ...
        private string GetInfoByCustomerNumberQuery(string dbName)
        {
            return @$"Select 
                    	town as ZoneId,
						t51.C2 ZoneTitle,
