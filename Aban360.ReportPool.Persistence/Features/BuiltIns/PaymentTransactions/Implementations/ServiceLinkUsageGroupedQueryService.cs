...
namespace Aban360.ReportPool.Persistence.Features.BuiltIns.PaymentTransactions.Implementations
{
    internal sealed class ServiceLinkUsageGroupedQueryService : PaymentBase, IServiceLinkUsageGroupedQueryService
    {
        ...
        {
            string query = GetGroupedQuery(false, input.ZoneIds?.Any() == true, GroupingFields.UsageTitle);
           // string query = GetServiceLinkUsageGroupedQuery(input.ZoneIds?.Any() == true);
            ...
            IEnumerable<ServiceLinkWaterItemGroupedDataOutputDto> serviceLinkUsageGroupedData = await _sqlReportConnection.QueryAsync<ServiceLinkWaterItemGroupedDataOutputDto>(query, @params);
        ...
        private string GetServiceLinkUsageGroupedQuery(bool hasZone)
        {
