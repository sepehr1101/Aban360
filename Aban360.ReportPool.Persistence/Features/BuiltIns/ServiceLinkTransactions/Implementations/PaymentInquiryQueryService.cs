...
namespace Aban360.ReportPool.Persistence.Features.BuiltIns.ServiceLinkTransactions.Implementations
{
    internal sealed class PaymentInquiryQueryService : AbstractBaseConnection, IPaymentInquiryQueryService
    {
        ...
        public async Task<PaymentInquiryOutputDto> GetInfo(PaymentInquiryInputDto input)
        {
            ...
            PaymentInquiryOutputDto data = await _sqlReportConnection.QueryFirstOrDefaultAsync<PaymentInquiryOutputDto>(query, @params);

            if (data is not null)
                data.IsPayed = !string.IsNullOrWhiteSpace(data.PayDateJalali);
        ...
        private string getQuery(string dbName)
        {
            return @$"Select
                    	...
                    	v.cod_bank as BankCode,
                    	v.sh_ghabs as BillId,
                    	v.sh_pard as PaymentId
