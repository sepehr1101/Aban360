...
{
    internal sealed class WaterReturnSummaryByUsageGroupQueryService : WaterReturnBase, IWaterReturnSummaryByUsageGroupQueryService
    {
        public WaterReturnSummaryByUsageGroupQueryService(IConfiguration configuration)
            : base(configuration)
        {
        }

        public async Task<ReportOutput<WaterReturnSummaryHeaderOutputDto, WaterReturnSummaryDataOutputDto>> Get(WaterReturnSummaryByUsageGroupInputDto input)
        {
            string reportTitle = ReportLiterals.WaterReturnSummary + GetIsZoneOrVillageTitle(input.ZoneIds);
            string WaterReturnSummarys = GetSummaryQuery(true, input.ZoneIds.HasValue(), false, input.BranchTypeIds.HasValue(), input.EnumInput);
