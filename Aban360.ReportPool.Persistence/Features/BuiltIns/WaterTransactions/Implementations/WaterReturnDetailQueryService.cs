...
{
    internal sealed class WaterReturnDetailQueryService : WaterReturnBase,  IWaterReturnDetailQueryService
    {
        public WaterReturnDetailQueryService(IConfiguration configuration)
            : base(configuration)
        {
        }

        public async Task<ReportOutput<WaterReturnDetailHeaderOutputDto, WaterReturnDetailDataOutputDto>> Get(WaterReturnDetailInputDto input)
        {
            string reportTitle = ReportLiterals.WaterReturnDetail + GetIsZoneOrVillageTitle(input.ZoneIds);
            string WaterReturnDetails = GetDetailQuery(input.ZoneIds.HasValue(), input.UsageIds.HasValue(), input.BranchTypeIds.HasValue());
            var @params = new
            {
            ...
            IEnumerable<WaterReturnDetailDataOutputDto> WaterReturnData = await _sqlReportConnection.QueryAsync<WaterReturnDetailDataOutputDto>(WaterReturnDetails, @params);
            WaterReturnDetailHeaderOutputDto WaterReturnHeader = new WaterReturnDetailHeaderOutputDto()
            {
                Title = reportTitle,
                ReportDateJalali = DateTime.Now.ToShortPersianDateString(),
                SumBillCount = WaterReturnData.Count(),
                RecordCount = WaterReturnData.Count(),
                CustomerCount = WaterReturnData.GroupBy(r => r.BillId).Distinct().Count(),
            ...
            var result = new ReportOutput<WaterReturnDetailHeaderOutputDto, WaterReturnDetailDataOutputDto>(reportTitle, WaterReturnHeader, WaterReturnData);
            return result;
        }
    }
}
