    ...
    internal abstract class ReadingStatusStatementBase : AbstractBaseConnection
    {
        ...
        internal string GetDetailQuery(bool isRegisterDate)
        {
            ...
            return @$"Select 
                    	...
                    	COUNT(Case When b.CounterStateCode NOT IN (4,7,8) AND c.DeletionStateId<>5 Then 1 End)AS PureReading,
                        COUNT(Case When b.CounterStateCode NOT IN (4,7,8) AND c.DeletionStateId=5 Then 1 End)AS PureReadingRemoved,
                    	COUNT(Case When b.CounterStateCode=4 AND c.DeletionStateId<>5 Then 1 End)AS Closed,
                        COUNT(Case When b.CounterStateCode=4 AND c.DeletionStateId=5 Then 1 End) AS ClosedRemoved,
