...
{
    internal sealed class SmsFlowUpdateHandler : AbstractBaseConnection, ISmsFlowUpdateHandler
    {
        private readonly IValidator<SmsFlowUpdateDto> _validator;
        public SmsFlowUpdateHandler(
            IValidator<SmsFlowUpdateDto> validator,
        ...
        public async Task Handle(SmsFlowUpdateDto inputDto, IAppUser appUser, CancellationToken cancellationToken)
        {
            await InputValidate(inputDto, cancellationToken);
            await ExecSql(inputDto);
        }
        private async Task ExecSql(SmsFlowUpdateDto inputDto)
        {
        ...
        private async Task InputValidate(SmsFlowUpdateDto input, CancellationToken cancellationToken)
        {
