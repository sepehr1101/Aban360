...
{
    internal sealed class SupportedOperatorGetAllHandler : ISupportedOperatorGetAllHandler
    {
        private readonly IMapper _mapper;
        private readonly ISupportedOperatorQueryService _supportedOperatorQueryService;
        public SupportedOperatorGetAllHandler(
            IMapper mapper,
        ...
        public async Task<ICollection<SupportedOperatorGetDto>> Handle(CancellationToken cancellationToken)
        {
            var supportedOperator = await _supportedOperatorQueryService.Get();
            return _mapper.Map<ICollection<SupportedOperatorGetDto>>(supportedOperator);
        }
    }
}
