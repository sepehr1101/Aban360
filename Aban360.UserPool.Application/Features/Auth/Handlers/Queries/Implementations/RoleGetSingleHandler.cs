...
{
    internal sealed class RoleGetSingleHandler : IRoleGetSingleHandler
    {
        ...
        {
            Role role = await _roleQueryService.Get(id);
            RoleGetDto dto= _mapper.Map<RoleGetDto>(role);
        ...
        private async Task<AccessTreeValueKeyDto> CreateAccessTree(ICollection<int> endpointIds)
        {
            ...
            AccessTreeValueKeyDto accessTree = endpoints.CreateAccessTree(endpointIds);
            return accessTree;
        }
    }
}
