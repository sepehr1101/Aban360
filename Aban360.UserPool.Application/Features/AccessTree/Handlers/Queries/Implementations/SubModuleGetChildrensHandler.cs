...
{
    internal sealed class SubModuleGetChildrensHandler : ISubModuleGetChildrensHandler
    {
        private readonly IMapper _mapper;
        private readonly ISubModuleQueryService _subModuleQueryService;
        public SubModuleGetChildrensHandler(
            IMapper mapper,
        ...
        public async Task<IEnumerable<SubModuleWithEndPointGetDto>> Handle(int id, CancellationToken cancellationToken)
        {
            IEnumerable<SubModule> SubModule = await _subModuleQueryService.GetChildrens(id);
            return _mapper.Map<ICollection<SubModuleWithEndPointGetDto>>(SubModule);
        }
    }
}
