...
namespace Aban360.UserPool.Persistence.Features.Auth.Queries.Implementations
{
    internal sealed class UserQueryService : IUserQueryService
    {
        ...
        private string GetSearchByRoleTitleAndZoneIdsDictionaryQuery(UserSearchByRoleTitleAndZoneIdsDto _params)
        {
            string zoneIds = string.Join(",", _params.ZoneIds);
            return @$"Select Distinct 
                    	u.Username Id,
                    	u.FullName Title
                    From Aban360.UserPool.UserClaim uc
