...
{
    internal sealed class GetFilesRequest : IGetFilesRequest
    {
        private readonly IOpenKmQueryService _openKmQueryService;
        public GetFilesRequest(IOpenKmQueryService openKmQueryService)
        {
        ...
        {
            //TODO: validate trackNumber is valid

            string folderName = $"r_{input}";
            FileListResponse result = await _openKmQueryService.GetFilesTracknumber(folderName);
            return result;
        }
    }
}
